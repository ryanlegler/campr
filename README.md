Load Collection from firebase
  Collection is a hashmap object store based on the bandcamp slug

  each album has a list of collectors
    each collector has a list of other albums they have purchashed


    how may different albums does the collector have in common with albums in my collection?
    --------------
    find the collectors for a given album
    rank the collector by how many albums the collector has in common with me.
    find other albums that collector owns add to related bucket - give a rank score based on the collector rank


    how may different collectors in this graph also purchased this album?
    ------------------------
    do the same for each of the collectors of an album
    if an album is already in the bucket then incrememnt the rank for that album
